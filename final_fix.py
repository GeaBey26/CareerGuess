...
file_path = r'c:\Users\Monster\OneDrive\Masaüstü\Antrigravitiy WS\script.js'

with open(file_path, 'rb') as f:
    content_bytes = f.read()

# We need to handle the content as strings that might be double-encoded
# or just mangled. Let's try to decode as utf-8 first, replacing errors.
content = content_bytes.decode('utf-8', errors='replace')

for mangled, correct in mapping.items():
    content = content.replace(mangled, correct)

# Also fix the TRANSLATIONS block specifically if it's still messed up
# Actually, I'll just replace the whole TRANSLATIONS block with a clean version
# because it was the most sensitive part.
...
# Find the start and end of the TRANSLATIONS object in the content
start_marker = "const TRANSLATIONS ="
end_marker = "class Game {"

start_idx = content.find(start_marker)
end_idx = content.find(end_marker)

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + translations_clean + "\n\n" + content[end_idx:]

# Also fix setLanguage method to be simple again
...
content = re.sub(set_lang_pattern, set_lang_replacement, content, flags=re.DOTALL)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Successfully fixed encoding and translations in script.js")
